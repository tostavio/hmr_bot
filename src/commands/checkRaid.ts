...
async function execute(interaction: CommandInteraction) {
  ...
  if (
    interactionChannel?.type === ChannelType.GuildText ||
    interactionChannel?.type === ChannelType.GuildAnnouncement
  ) {
    ...
    try {
      const confirmation = await response?.awaitMessageComponent({
        filter: (i) =>
          i.customId === "remove_dkp_role" && i.user.id === interaction.user.id,
      });
...
async function removeUsersFromRole(
  userIds: string[],
  eventId: string,
  ...
  console.log(
    `[LOG] Removendo role DKP dos usuários não presentes no evento: ${userIds.length} usuários.`
  );
  ...
  async function removeRole(member: GuildMember) {
    if (!member || !role) {
    ...
    const progress = Math.min(
      Math.floor((processedUsers / totalUsers) * 100),
      100
